...
def find_aequidist_points_on_sphere(p1, p2, r, deg: bool = True, R=1):
    """
    Gets two points p1 and p2 and finds one oder two points on the sphere with radius R, which have the distance r from p1 and p2.
    :param p1: first point on sphere
    :param p2: second point on sphere
    :param r: radius on the sphere in deg or rad
    :param deg: flag which indicates, that r is a degree
    :param R: big radius R of the sphere
    :return: two points on the sphere, which are r distant from p1 and p2. throws RuntimeErrror if points do not exists
    """

    if deg == True:
        r = np.deg2rad(r)
    # calculate the euclidian distance from radiance
    r = 2 * R * np.sin(r / 2)
    # Origin of the main sphere
    p0 = [0, 0, 0]

    spheres = [Sphere(p0, R), Sphere(p1, r), Sphere(p2, r)]
...
class Sphere:
    def __init__(self, center, radius):
        ...
        :param center: 3d np array for center.
    ...
    def find_intersection_with_circle(self, circle):
        ...
        h, c, u, v = circle
        cd = c - self.center
        # print(np.square(self.radius), -np.dot(cd, cd), -np.square(h))
