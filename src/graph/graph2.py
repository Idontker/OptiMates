...
def save(g2: Graph2, filepath: str) -> None:
    np.save(file=filepath + "-points.npy", arr=g2.points)
    f = open(file=filepath + "-settings.txt", mode="w", newline="")
    f.write("number of points:" + str(g2.number_of_points) + "\n")
    f.write("cover radius:" + str(g2.cover_radius))
    f.flush()
    f.close()
    pass


def load_graph_from(filepath: str) -> Graph2:
    try:
    ...
    g2 = Graph2(cover_radius=r, number_of_points=N)
    g2.points = np.load(file=filepath + "-points.npy")
    return g2
